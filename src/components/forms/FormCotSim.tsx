...
export default function FormularioCotacaoSimulacao() {
    ...
    const rhf = useForm<CotacaoDados>({
        ...
        defaultValues: {
            ...
            difalOpcao: false,
            cubagens: [
                { quantidade: "", comprimento: "", largura: "", altura: "" },
            ],
        },
    });

    //useState solicitante verificado
    const [solicitanteVerificado, setSolicitanteVerificado] =
    ...
    //Dados da cotação
    const [cotacaoDados, setCotacaoDados] = useState<CotacaoResponse | null>(
        null,
    );

    //useState para verificar se VIACEP está ativo ou não
    const [apiCepCheck, setApiCepCheck] = useState<
        "carregando" | "online" | "offline"
    >("carregando");
    ...
    useEffect(() => {
        function aoFecharAba() {
            ...
            const valores = [getValues(), cotacaoDados || null];
            ...
            if (solicitanteVerificado && !cotacaoCompleta) {
                ...
                const payload = new Blob([JSON.stringify(valores)], {
    ...
    useEffect(() => {
        async function checarApi() {
            try {
                ...
                if (
                    resposta.cepValido === true ||
                    resposta.cepValido === false
                ) {
    ...
    async function handlerSubmeterCotacao(dadosFormulario: CotacaoDados) {
        ...
        try {
            ...
            const { cepValido: cepValidoDestino } = await validarCep(
                dadosFormulario.cepDestino,
            );
            ...
            const { cepValido: cepValidoOrigem } = await validarCep(
                dadosFormulario.cepOrigem,
            );
    ...
    if (apiCepCheck === "online") {
        return (
            <>
                <FormProvider {...rhf}>
                    <motion.div
                        className="bg-white w-full  py-5 px-7 rounded-xl shadow-lg overflow-hidden"
                        initial={{ height: 50 }}
                        animate={{ height: "auto" }}
                    ...
                    <AnimatePresence>
                        {cotacaoDados && (
                            <CotacaoCard
                                ...
                                clicado={cotacaoCompleta}
                            />
                        )}

                        {/* Verifique a variavel de ambiente */}
                        {COT_COMPLETO && cotacaoCompleta && (
                            <FormularioCotacaoCompleto
                                dadosSimulacao={getValues()}
                                key={"formulario-completo"}
                            />
                        )}
                    </AnimatePresence>
                </FormProvider>
        ...
        return <span>Site em manutenção, tente novamente mais tarde...</span>;
